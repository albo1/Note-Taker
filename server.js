...
app.delete('/api/notes/:id', (req, res) => {
    ...
    fs.readFile('./db/db.json', 'utf-8', (err, data) => {
        if (err) {
            ...
            const notesParsed = JSON.parse(data)
            const newNotesList = notesParsed.filter(note => note.id !== noteId)

            fs.writeFile('./db/db.json', JSON.stringify(newNotesList),
                (writeError) =>
                    writeError ? console.error(writeError) : console.info('Successfully updated notes!')
            );
        }
    });
